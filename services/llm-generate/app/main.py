...
Observability:
- Emits structured events for Generation with model, prompt/output sizes.
- Uses tracing spans around LLM calls.
...
async def summarize_document(request: SummarizeRequest) -> SummarizeResponse:
    ...
    try:
        ...
        with span(
            ...
            prompt_tokens=estimate_tokens(context_str),
            corr=corr,
        ):
            resp = model.generate_content(
                [
        ...
        if text:
            ...
            log_event(
                ...
                payload={
                    ...
                    "model": s.gemini_fast_model,
                    "prompt_tokens": estimate_tokens(context_str),
                    "output_tokens": estimate_tokens(summary or text or ""),
                },
                correlation_id=corr,
            )
            return out
